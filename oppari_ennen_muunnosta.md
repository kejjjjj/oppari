...
Tapaustutkimus valittiin tutkimusmenetelmäksi, koska työn tavoitteena on tarkastella Markdownin soveltuvuutta käytännön kontekstissa eikä tuottaa tilastollisesti yleistettävää tietoa. Vaihtoehtoisena lähestymistapana olisi voitu käyttää esimerkiksi kyselytutkimusta, mutta se ei olisi mahdollistanut työkalujen ja prosessien konkreettista arviointia. Tapaustutkimus soveltuu tähän työhön, koska se mahdollistaa syvällisen tarkastelun rajatussa ympäristössä.
...
Tutkimuskysymysten avulla tutkimus keskittyy “miten” eikä pelkästään “miksi” -näkökulmaan. Tarkastelun kohteena ovat erityisesti työkalut ja menetelmät, kuten web scraping -ratkaisut, HTML:n jäsentämiseen käytettävät kirjastot, Markdown-muunnostyökalut sekä staattisten sivustojen generaattorit.
...
Vaikka Markdown -> DOCX -muunnos tuottaa rakenteellisesti oikeanlaisen dokumentin, opinnäytetyö vaatii lisäksi tarkasti määritellyn ulkoasun. Tämä tarkoittaa, että pelkkä sisällön muuntaminen ei riitä, vaan dokumentin muotoilu on sovitettava organisaation opinnäytetyöpohjaan. Tässä työssä käytetään HAMKin Word-mallipohjaa, joka sisältää valmiiksi määritellyt tyylit, kuten otsikkotasot, leipätekstin asetukset, rivivälit ja marginaalit.

Muotoilun sovittaminen toteutetaan ensisijaisesti Pandocin tarjoaman `--reference-doc`-parametrin avulla. Tämän avulla muunnosvaiheessa voidaan määrittää Word-dokumentti, jonka tyylejä käytetään lopullisessa tiedostossa.

```
pandoc oppari.md -o oppari.docx --reference-doc=hamk_pohja.docx
```

*Ohjelmakoodi 20.* Havainnollistaa mallipohjan hyödyntämistä muunnoksessa.

Käytännössä tämä tarkoittaa, että Pandoc ottaa mallipohjasta esimerkiksi “Heading 1”, “Heading 2” ja “Normal” -tyylit ja soveltaa niitä Markdown-dokumentin vastaaviin rakenteisiin.

Muunnos toteutetaan siten, että Markdown-tiedosto konvertoidaan Word-muotoon käyttäen valmista mallipohjaa, jonka oletetaan sisältävän oikeat tyylit, otsikkorakenteet sekä sivuasetukset. Käytännössä tämä tarkoittaa, että Pandoc soveltaa mallipohjan tyylejä Markdown-dokumentin elementteihin muunnosvaiheessa. Teoriassa tämän pitäisi tuottaa suoraan vaatimusten mukainen dokumentti, mutta käytännön kokeilussa ilmenee useita ongelmia.

Yksi havainto liittyy ylätunnisteisiin (footer). Opinnäytetyöpohjassa ensimmäisellä sivulla käytetään erilaista ylätunnistetta kuin muilla sivuilla, mikä on toteutettu Wordissa erillisten osioiden avulla. Pandoc ei kuitenkaan säilytä tätä rakennetta oikein muunnosvaiheessa, vaan kopioi ensimmäisen sivun alatunnisteen kaikille sivuille. Tämän seurauksena dokumentin sivunumerointi ja alatunnisteet eivät vastaa vaadittua rakennetta, ja ne joudutaan korjaamaan manuaalisesti Wordissa.

Toinen merkittävä ongelma liittyy otsikoiden numerointiin. Alkuperäinen Markdown-dokumentti sisältää jo valmiiksi määritellyn otsikkorakenteen ja numeroinnin, mutta Word-mallipohja saattaa sisältää omat automaattiset numerointiasetuksensa otsikkotyyleille. Muunnoksen yhteydessä nämä asetukset aktivoituvat, mikä johtaa siihen, että otsikot numeroidaan uudelleen. Tämä rikkoo alkuperäisen numeroinnin ja voi aiheuttaa epäjohdonmukaisuuksia dokumentin rakenteessa.

*Kuva 10.* Havainnollistaa rikkinäistä Word-muunnosta.

![alt text](image-1.png)

Näiden ongelmien seurauksena pelkkä automaattinen muunnos ei riitä tuottamaan suoraan valmista opinnäytetyödokumenttia. Käytännössä dokumentti joudutaan avaamaan Wordissa ja korjaamaan käsin ainakin seuraavien osa-alueiden osalta: alatunnisteiden rakenne, sivunumerointi sekä otsikoiden numerointi. Tämä heikentää automaation hyötyjä ja lisää manuaalisen työn määrää erityisesti viimeistelyvaiheessa.

Samalla kuitenkin havaitaan, että osa muotoilusta siirtyy onnistuneesti. Esimerkiksi otsikkotyylit, kappalejako ja perusrakenne säilyvät pääosin oikeina, mikä vähentää kokonaisuudessaan tarvittavaa muokkaustyötä. Näin ollen muunnos ei ole täysin epäonnistunut, mutta se ei myöskään täytä täysin opinnäytetyöpohjan vaatimuksia ilman lisätoimenpiteitä.
